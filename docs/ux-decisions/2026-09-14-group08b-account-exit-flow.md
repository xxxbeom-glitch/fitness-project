...
- no right action
- `탈퇴 전에 확인해 주세요`
- 삭제되는 데이터 표시는 행/카드로 나누지 않고 쉼표 구분 한 문장으로 노출:
  - `운동 기록, 루틴, 직접 만든 운동, 프로필 및 신체 정보, 앱 설정 및 계정 데이터`
- 개인정보 파기 대상도 서술형 설명 대신 쉼표 구분으로 간단히 노출:
  - `닉네임, 프로필 이미지, 성별, 생년월일, 신체 정보, 소셜 로그인 연동 정보`
...
- title = `정말 계정을 탈퇴할까요?`
- 운동 기록, 루틴, 프로필과 개인정보 등 모든 계정 데이터가 삭제되며 복구할 수 없다는 안내
- actions = `취소 / 탈퇴하기`
...
- 개인정보 파기 대상은 `닉네임, 프로필 이미지, 성별, 생년월일, 신체 정보, 소셜 로그인 연동 정보`로 정의한다.
...
- 삭제 데이터/개인정보 파기 대상이 쉼표 구분 한 문장으로 표시되는 것 확인
- 복구 불가/재가입 안내 확인
- bottom CTA 확인
