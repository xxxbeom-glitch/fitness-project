...
- `내 루틴` is persistent below the primary Home section: 02A = empty state, 02B = 2 × n grid; 02D keeps My Routine below the active-workout section
...
| Screen | Purpose / Entry | Primary behavior / Exit | Implementation rule | QA |
|---|---|---|---|---|
| `02A_Home_NoRoutine` | Home with no saved routine | `빈 운동` → zero-exercise Active Workout; `내 루틴 만들기` / `내 루틴 > 새 루틴` → routine create | Quick Start + persistent `내 루틴` empty state. No Recent Workout section. Blank workout creates no saved routine. | PASS |
...
| `02D_Home_Active` | Home while one active workout exists | active card → resume same session; `내 루틴` remains below | Active workout is the priority section; My Routine persists and switches empty/populated by saved-routine count. No Recent Workout section. Never create a second active session silently. | PASS |
