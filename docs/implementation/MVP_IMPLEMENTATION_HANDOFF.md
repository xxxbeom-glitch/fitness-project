...
Completion PR presentation:
- no valid PR → hide the PR card entirely; no empty/failure placeholder
- one or more valid PRs → show all valid PR rows generated by that saved session inside the single shared PR card
- order by workout-session exercise display order
- use native recording-type formatting
- do not collapse multiple valid PRs to one representative row
- do not use `외 N개`
...
Group 07 navigation:
- 07A body-area row → matching `07B_BodyArea_Detail`
...
- columns = `운동 / 수행 / 세트`
- group identical performance combinations by set count
- keep different performance combinations as separate rows
- use recording-type-native performance formatting
- do not invent a cross-recording-type unit conversion
- do not replace this with per-exercise cards
