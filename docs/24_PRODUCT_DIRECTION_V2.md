...
- 루틴 없음 -> 상단 `빈 운동` / `내 루틴 만들기`, 하단 `내 루틴` empty state
...
Home에는 최근 운동 섹션을 두지 않는다. 대신 `내 루틴` 섹션은 routine count와 무관하게 유지하며, 0개일 때는 empty state를 보여준다. 운동 이력은 분석/히스토리 영역에서 확인한다. **Home의 최상위 역할은 운동 시작/복귀**다.
