...
Home에는 최근 운동 섹션을 두지 않는다. `내 루틴` 섹션은 routine count와 무관하게 유지하되, 0개일 때는 `내 루틴` 헤더만 남기고 별도 empty 문구/본문을 보여주지 않는다. `내 루틴` 헤더 우측 액션도 사용하지 않으며 루틴 생성은 상단 `내 루틴 만들기`에서 한다. 운동 이력은 분석/히스토리 영역에서 확인한다. **Home의 최상위 역할은 운동 시작/복귀**다.
